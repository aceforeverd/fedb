...
void date_diff(StringRef *date1, StringRef *date2, int64_t *diff, bool *is_null) {
...
void date_diff(StringRef *date1, Date *date2, int64_t *diff, bool *is_null) {
    auto t1 = string_to_time(absl::string_view(date1->data_, date1->size_));
    if (!t1.ok()) {
        *is_null = true;
        return;
...
void date_diff(Date *date1, StringRef *date2, int64_t *diff, bool *is_null) {
    auto t2 = string_to_time(absl::string_view(date2->data_, date2->size_));
    if (!t2.ok()) {
        *is_null = true;
        return;
